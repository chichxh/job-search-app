...
## Сценарий скриншотов для отчёта ВКР

1. **/login** — экран входа без технических сообщений и debug-информации.
2. **/register** — экран регистрации с минимальными полями.
3. **/settings** — только два блока:
   - авторизация HeadHunter;
   - импорт профиля из текста/файла резюме.
4. **/vacancies** — кнопка **«Загрузить демо-вакансии»** и список вакансий.
5. **/recommendations** — кнопка **«Пересчитать рекомендации»** и ранжированный список.
6. **/vacancies/:vacancyId** — в одном кадре:
   - данные вакансии;
   - блок мэтчинга (score/verdict, совпавшие и недостающие требования, рекомендации);
   - генерация и подтверждение резюме/сопроводительного;
   - запуск/отслеживание отклика.
7. **/applications** — в одном кадре:
   - воронка по статусам;
   - summary-блок;
   - карточки откликов;
   - изменение статуса и история статусов.

Рекомендации перед съёмкой:
- использовать demo-данные (без реальных персональных данных);
- убрать лишние вкладки браузера и системные уведомления;
- оставлять только пользовательские элементы интерфейса.
