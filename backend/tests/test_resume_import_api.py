...
def test_resume_extract_txt_happy_path(client, auth_headers):
    with _open_fixture("sample_resume.txt") as file_handle:
        response = client.post(
            "/api/v1/profiles/1/resume-import/extract",
            headers=auth_headers,
            files={"file": ("sample_resume.txt", file_handle, "text/plain")},
        )
...
def test_resume_extract_pdf_happy_path(client, auth_headers):
    with _open_fixture("sample_resume.pdf") as file_handle:
        response = client.post(
            "/api/v1/profiles/1/resume-import/extract",
            headers=auth_headers,
            files={"file": ("sample_resume.pdf", file_handle, "application/pdf")},
        )
...
def test_resume_extract_pdf_with_no_extractable_text_returns_422(client, auth_headers):
    with _open_fixture("no_text_resume.pdf") as file_handle:
        response = client.post(
            "/api/v1/profiles/1/resume-import/extract",
            headers=auth_headers,
            files={"file": ("no_text_resume.pdf", file_handle, "application/pdf")},
        )
...
def test_resume_extract_near_empty_text_returns_422(client, auth_headers):
    with _open_fixture("low_signal_resume.txt") as file_handle:
        response = client.post(
            "/api/v1/profiles/1/resume-import/extract",
            headers=auth_headers,
            files={"file": ("low_signal_resume.txt", file_handle, "text/plain")},
        )
