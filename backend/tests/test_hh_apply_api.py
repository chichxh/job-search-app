...
class FakeApplyAutomationClient(HHApplyAutomationClient):
    def __init__(self, *, should_fail: bool = False, retryable: bool = True) -> None:
    ...
    def apply_to_vacancy(self, *, user_id, connection, apply_run, managed_resume, vacancy, cover_letter_text, dry_run):
        ...
        return HHApplyAutomationResult(
            result_type="dry_run" if dry_run else "submitted",
...
def _override_service(fake_db, *, should_fail: bool = False, retryable: bool = True):
    def _factory_override():
        return HHApplyService(
            fake_db,
            automation_client=FakeApplyAutomationClient(should_fail=should_fail, retryable=retryable),
        )
