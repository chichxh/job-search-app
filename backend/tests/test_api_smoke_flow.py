...
def test_health_and_profile_read_write(client):
...
def test_recommendations_endpoint_returns_expected_structure(client, fake_db):
    ...
    response = client.get("/api/v1/profiles/1/recommendations")
...
def test_tailoring_endpoint_returns_explanation_and_evidence(client, fake_db, monkeypatch):
    fake_db.queue_execute_results(
    ...
    response = client.get("/api/v1/profiles/1/vacancies/1/tailoring")
    assert response.status_code == 200
    payload = response.json()
...
def test_docgen_happy_path_and_approvals(client, monkeypatch):
    ...
    generated = client.post("/api/v1/profiles/1/vacancies/1/resume/generate")
    ...
    resume_approved = client.post(f"/api/v1/profiles/1/resume-versions/{resume_id}/approve")
    ...
    cover_approved = client.post(f"/api/v1/profiles/1/cover-letter-versions/{cover_id}/approve")
    assert cover_approved.status_code == 200
    assert cover_approved.json()["status"] == "approved"


def test_docgen_provider_failure_is_normalized(client, monkeypatch):
    ...
    response = client.post("/api/v1/profiles/1/vacancies/1/cover-letter/generate")
