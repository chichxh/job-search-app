...
def test_change_status_writes_history(client, auth_headers):
    created = client.post("/api/v1/profiles/1/applications", json={"vacancy_id": 1}, headers=auth_headers)
    ...
    history_response = client.get(f"/api/v1/profiles/1/applications/{application_id}/history", headers=auth_headers)
...
def test_history_endpoint_returns_ordered_events(client, auth_headers):
    created = client.post("/api/v1/profiles/1/applications", json={"vacancy_id": 1}, headers=auth_headers)
    assert created.status_code == 201
    application_id = created.json()["id"]
...
def test_attach_document_validates_profile_ownership(client, fake_db, auth_headers):
    fake_db.add(
